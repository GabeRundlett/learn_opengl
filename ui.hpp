...
#include "opengl/renderers/ui_batch.hpp"
#include "opengl/renderers/text_batch.hpp"
...
struct slider {
    ...
    slider_range range;
    float value = 0.5f, prev_value = 0.5f;
    bool is_hovered = false, is_pressed = false;
    ...
    void draw(opengl::renderer::ui_batch &batch, opengl::renderer::text_batch &text_batch) const {
        batch.submit_rect(pos, size, is_hovered ? glm::vec4{0.15, 0.15, 0.3, 1} : glm::vec4{0.1, 0.1, 0.2, 1});
        batch.submit_rect(pos, {size.x * value, size.y}, is_hovered ? glm::vec4{0.5, 0.5, 0.8, 1} : glm::vec4{0.4, 0.4, 0.7, 1});
        text_batch.submit(pos + label.offset, label.text.c_str(), label.scale, is_hovered ? label.color_focused : label.color_unfocused);
    }
};
...
struct checkbox {
    ...
    void draw(opengl::renderer::ui_batch &batch) const {
        if (is_hovered)
            batch.submit_rect(pos - size * 0.1f, size * 1.2f, glm::vec4{0.4, 0.4, 0.6, 1});
        batch.submit_rect(pos, size, glm::vec4{0.1, 0.1, 0.2, 1});
        if (is_checked)
            batch.submit_rect(pos + size * 0.1f, size * 0.8f, glm::vec4{0.4, 0.4, 0.7, 1});
    }
};
...
struct textbox {
    ...
    void draw(opengl::renderer::ui_batch &batch, opengl::renderer::text_batch &text) const {
        if (is_hovered)
            batch.submit_rect(pos - glm::vec2{1, 1}, size + glm::vec2{2, 2}, glm::vec4{0.4, 0.4, 0.6, 1});
        batch.submit_rect(pos, size, glm::vec4{0.1, 0.1, 0.2, 1});
        if (is_focused) {
            batch.submit_rect(pos + glm::vec2{1, 1}, size - glm::vec2{2, 2}, glm::vec4{0.2, 0.2, 0.4, 1});
            auto cursor_offset = calc_offset(contents, cursor_index, text);
            batch.submit_rect(pos + cursor_offset + glm::vec2{0, 2}, glm::vec2{1, 16}, glm::vec4{1, 1, 1, 1});
        }
...
struct menu_ui {
    ...
    bool process_key(int button, int scancode, int action, int mods) {
        for (auto &elem : textboxes)
            if (elem.process_key(button, scancode, action, mods))
                return true;
        return false;
    }
    ...
    void draw_debug() {
        // float menu_width = is_paused ? 200 : 0;
        // text.begin();
        // text.submit({10 + menu_width, 20}, fmt::format("resolution [{}, {}]", frame_dim.x, frame_dim.y), 14, {1, 1, 1, 1});
        // text.submit({10 + menu_width, 38}, fmt::format("render resolution [{}, {}] ({}x)", scene_frame_dim.x, scene_frame_dim.y, render_resolution_scale * render_resolution_scale), 14, {1, 1, 1, 1});
        // text.end();
        // text.flush();
    }
};
