...
struct test_scene {
    ...
    opengl::texture3d<glm::u8vec4> voxel_tex;
    ...
    test_scene()
        : cube_vao(),
          ...
          voxels(lattice_dim.x * lattice_dim.y * lattice_dim.z),
          voxel_tex({
              .data = nullptr,
    ...
    void init() {
        ...
        for (int z = 0; z < lattice_dim.z; ++z) {
            for (int y = 0; y < lattice_dim.y; ++y) {
                for (int x = 0; x < lattice_dim.x; ++x) {
